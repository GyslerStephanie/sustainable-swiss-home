...
import { rooms as ROOMS, finishById, type Listing, type PlanState } from "@/app/lib/data";
...
export function FloorPlan({ listing, state, view, selectedRoom, onSelectRoom }: FloorPlanProps) {
  const rooms = ROOMS;
  const after = view === "after";
  const sys = state.systems;
  const has = (id: string) => after && sys.includes(id);

  const OX = 16,
    OY = 16,
    OW = 568,
    OH = 438; // outer wall box (within viewBox)
    ...
    <svg className="plan-svg" viewBox="0 -40 600 514" xmlns="http://www.w3.org/2000/svg">
      ...
      <g>
        <rect
          ...
          stroke={has("roof") ? "var(--green)" : "var(--line-2)"}
          strokeWidth="1.5"
        />
        <text
          x={OX + 10}
        ...
        {has("pv") &&
          [0, 1, 2, 3, 4, 5].map((i) => (
            <rect key={i} x={OX + OW - 130 + i * 21} y={-28} width={17} height={13} rx={2} fill="var(--acc)" opacity={0.85} />
          ))}
        {has("pv") && (
          <text x={OX + OW - 138} y={-17} textAnchor="end" className="rarea" style={{ fontSize: 10, fill: "var(--acc-2)" }}>
            PV ☀
          </text>
        )}
      </g>
      ...
      {rooms.map((r) => {
        ...
        return (
          ...
          >
            <rect x={r.x} y={r.y} width={r.w} height={r.h} rx={2} fill={fill} fillOpacity={fillOpacity} />
            <rect className="rstroke" x={r.x} y={r.y} width={r.w} height={r.h} rx={2} />
            <text className="rlabel" x={r.x + r.w / 2} y={r.y + r.h / 2 - 2} textAnchor="middle">
              {r.name}
            </text>
            <text className="rarea" x={r.x + r.w / 2} y={r.y + r.h / 2 + 14} textAnchor="middle">
              {r.area} m²{after && r.finish && fid !== "keep" ? " · " + f.name : ""}
            </text>
          </g>
        );
      })}
      ...
      {has("windows") &&
        ...
        ].map((g, i) => <rect key={i} x={g.x} y={g.y} width={g.w} height={g.h} rx={2} fill="var(--acc)" opacity="0.9" />)}

      {/* technical-room system markers */}
      {has("heatpump") && (
        <g className="sys-marker">
          <rect x={478} y={170} width={42} height={26} rx={7} fill="var(--acc)" />
          <text x={499} y={187} textAnchor="middle" fill="#fff" fontSize="11">
            WP
          </text>
        </g>
      )}
      {has("mvhr") && (
        <g className="sys-marker">
          ...
          <text x={549} y={187} textAnchor="middle" fill="#fff" fontSize="11">
            LÜ
          </text>
        </g>
      )}
